...
const EditBlogPage = () => {
  ...
  const saveChanges = (payload: any) => {
    setLoading(true)
    try {
      axios.patch('https://blog.jointpowersecurity.com/server.php?id=' + router.query.blogId, payload, {
        headers: {
  ...
  useEffect(() => {
    axios.get('https://blog.jointpowersecurity.com/server.php?id=' + router.query.blogId).then((res: AxiosResponse) => {
